...
const moduleName = 'Position utils';
...
tape(
	`${moduleName}
	isSamePosition()
		WHEN called with two different positions`,
	(test: tape.Test) => {
		test.plan(1);
		test.equal(
...
tape(
	`${moduleName}
	isSamePosition()
		WHEN called with two identical positions`,
	(test: tape.Test) => {
		test.plan(1);
		test.equal(
...
tape(
	`${moduleName}
	getPositionText()
		WHEN called with a position`,
	(test: tape.Test) => {
		test.plan(1);
		test.equal(
			testedModule.getPositionText({ loggerService: loggerServiceFactory() })({
				position: { x: 4, y: 5, z: 6 },
			}),
			'{x: 4, y: 5, z: 6}',
			'SHOULD return a string representation of the position',
		);
		test.end();
	},
);
